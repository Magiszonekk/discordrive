...
```bash
npx tsx how-to-use/upload.ts ./any-file.txt
...
```bash
npx tsx how-to-use/upload.ts ./video.mp4
```
...
Example:
```bash
npx tsx how-to-use/download.ts clxabc123 "aGVsbG8..." "c29tZ..." ./video-copy.mp4
```
...
```bash
npm install @ddv4/processing @ddv4/config
...
```typescript
import { generateFEK, wrapKey, encryptChunk, toBase64, fromBase64, unwrapKey, decryptChunk, importKey } from "@ddv4/processing";
